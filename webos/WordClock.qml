...
Rectangle {
    function selectLanguage(language){
        if (language !== "") {
            language_url = "languages/%1.qml".arg(language)
    ...
    property url language_url: "languages/english.qml"
    ...
    Loader { source: language_url; onLoaded: {language = item; statusText.text = "loaded"}
        onStatusChanged: statusText.text = "status: " + (status==Loader.Error) }
    Timer {
